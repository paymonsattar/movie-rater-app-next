...
import { createMovie, getMovieById, getAllMovies } from '../../movies/src/handlers/movieHandlers'; // Replace with the actual module name
...
describe('Movies API handlers tests', () => {
  ...
  describe('createMovie', () => {
    ...
    it('should return 500 if an error occurs', async () => {
      mockRedisClient.HSET = jest.fn().mockRejectedValue(new Error('Redis error'));
      ...
      expect(res.json).toHaveBeenCalledWith({ message: 'Error creating movie' });
    });
  
    it('should return 400 if title is missing', async () => {
      req = mockRequest({ genre: 'Action', releaseDate: '2022-09-05', moviePoster: 'url' }, {});
  
      await createMovie(redisClient)(req, res);
  
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.json).toHaveBeenCalledWith({ message: 'title, genre, and releaseDate are required fields' });
    });
  
    it('should return 400 if genre is missing', async () => {
      req = mockRequest({ title: 'Movie1', releaseDate: '2022-09-05', moviePoster: 'url' }, {});
    ...
    it('should return 400 if releaseDate is missing', async () => {
      req = mockRequest({ title: 'Movie1', genre: 'Action', moviePoster: 'url' }, {});
    ...
    it('should return 400 for invalid title format', async () => {
      req = mockRequest({ title: {}, genre: 'Action', releaseDate: '2022-09-05', moviePoster: 'url' }, {});
  
      await createMovie(redisClient)(req, res);
  
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.json).toHaveBeenCalledWith({ message: 'Invalid data types: title and genre must be strings' });
    });
  
    it('should return 400 for invalid genre format', async () => {
      req = mockRequest({ title: 'Movie1', genre: {}, releaseDate: '2022-09-05', moviePoster: 'url' }, {});
    ...
    it('should return 400 for invalid releaseDate format', async () => {
      req = mockRequest({ title: 'Movie1', genre: 'Action', releaseDate: {}, moviePoster: 'url' }, {});
  
      await createMovie(redisClient)(req, res);
  
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.json).toHaveBeenCalledWith({ message: 'Invalid data type: releaseDate must be a date type' });
    });
  
    it('should return 400 for invalid moviePoster format', async () => {
      req = mockRequest({ title: 'Movie1', genre: 'Action', releaseDate: '2022-09-05', moviePoster: {} }, {});
  
      await createMovie(redisClient)(req, res);
  
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.json).toHaveBeenCalledWith({ message: 'Invalid data type: moviePoster must be a string' });
    });
  });
  ...
  describe('getAllMovies', () => {
    it('should successfully get all movies', async () => {
      ...
      mockRedisClient.LRANGE = jest.fn().mockResolvedValue(['{"id": "1", "title": "Movie1"}', '{"id": "2", "title": "Movie2"}']);
      ...
      expect(res.json).toHaveBeenCalledWith([{ id: "1", title: "Movie1" }, { id: "2", title: "Movie2" }]);
    });
    ...
    it('should return 500 if an error occurs', async () => {
      ...
      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith({ message: 'Error fetching movies' });
    });
  ...
  describe('getMovieById', () => {
    it('should successfully get a movie by ID', async () => {
      ...
      mockRedisClient.HGETALL = jest.fn().mockResolvedValue({ id: "1", title: "Movie1", genre: "Action", releaseDate: "2022-09-05", moviePoster: "url" });
      ...
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({ id: "1", title: "Movie1", genre: "Action", releaseDate: "2022-09-05", moviePoster: "url" });
    });
  
    it('should return 500 if an error occurs', async () => {
      // Mocking a Redis error
      mockRedisClient.HGETALL = jest.fn().mockRejectedValue(new Error('Redis error'));
      ...
      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith({ message: 'Error fetching movie' });
    });
