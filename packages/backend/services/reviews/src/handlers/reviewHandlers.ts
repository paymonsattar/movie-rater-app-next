...
export const addReview = (client: RedisClient) => async (req: Request, res: Response) => {
  ...
  if (isNaN(reviewNumber) || reviewNumber < 0 || reviewNumber > 5) {
    return res.status(400).json({ error: 'Invalid review value, must be between 0 and 5' });
  }

  try {
    const reply = await client.RPUSH(`reviews:${movieId}`, reviewNumber.toString());
...
export const getReviews = (client: RedisClient) => async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!reply || reply.length === 0) {
      return res.status(404).json({ error: 'No reviews found for this movieId' });
    }
