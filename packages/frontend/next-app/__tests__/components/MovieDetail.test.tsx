...
describe('MovieDetail', () => {
  beforeAll(() => {
    ...
    .reply(200, { /* your mocked response object for reviews */ });

    nock('http://localhost:3002')
    .defaultReplyHeaders(nockCORSHeaders)
    .post('/reviews/')
    .reply(200, { /* your mocked response object */ });
  });
  ...
  it('Should display the movie title.', () => {
    const { getByText } = render(<MovieDetail movie={mockMovie} onRatingSubmit={mockOnRatingSubmit} />);
  ...
  it("Should display the movie's image.", () => {
    const { getByAltText } = render(<MovieDetail movie={mockMovie} onRatingSubmit={mockOnRatingSubmit} />);
    expect(getByAltText('Mock Movie Title poster')).toHaveAttribute('src', '/_next/image?url=%2Fmock_movie_picture.webp&w=1080&q=75');
  });
  ...
  it("Should display the movie's genres.", () => {
    const { getAllByText } = render(<MovieDetail movie={mockMovie} onRatingSubmit={mockOnRatingSubmit} />);
  ...
  it("Should show a RatingsComponent to rate the movie.", () => {
    const { getByTestId } = render(<MovieDetail movie={mockMovie} onRatingSubmit={mockOnRatingSubmit} />);
