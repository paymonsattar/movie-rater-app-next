  ...
  return (
    <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700 flex flex-col justify-between">
      <div className="relative flex justify-center">
        <div className="absolute left-0 w-full h-8 bg-blue-900 transform rounded-t-lg"></div>
        <h5 className="relative z-10 px-8 py-2 text-xl font-semibold tracking-tight text-white bg-blue-900 rounded-md">
          {title}
        </h5>
      </div>
      ...
      <div className="px-5 pb-5">
        <div className="flex flex-wrap mt-2 mb-2">
          {genres.map((genre, index) => (
            <span key={index} className="m-1 p-2 rounded-lg bg-gray-600 text-white text-sm">
              {genre}
            </span>
          ))}
        </div>
